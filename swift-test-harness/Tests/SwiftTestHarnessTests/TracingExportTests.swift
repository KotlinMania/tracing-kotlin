...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Tracing.swiftmodule/`
...
//      `import Tracing` above would fail at compile time.
//
//   2. The static archive `libTracing.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Tracing" }` and
//      `flattenPackage = "io.github.kotlinmania.tracing"` configuration in
...
//      via the `TracingLibrary` product.
//
...
final class TracingExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Tracing swift module imported cleanly")
    }
}
