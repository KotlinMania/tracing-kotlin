# Claude Code Project Instructions — tracing-kotlin

## Project Overview

This is **tracing-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`tracing`](https://crates.io/crates/tracing).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/tracing/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/tracing>
...
Targets: macOS arm64, Linux x64, mingw-x64, iOS arm64/simulator-arm64, JS, Wasm-JS, Android.
